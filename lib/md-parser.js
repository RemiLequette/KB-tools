...
function _tocEntryLines(sectionName, sectionLines) {
  const entryLines = [`- [[#${sectionName}]]`];
  let inFence = false;
  for (const raw of sectionLines) {
    ...
    if (subMatch) entryLines.push(`  - [[#${subMatch[1].trim()}]]`);
  }
  return entryLines;
}

/** Builds the `## Table of Contents` heading and its wikilink list lines (including the trailing blank line), from the document's current sections. */
function buildTocLines(doc) {
  const lines = ['## Table of Contents', ''];
  for (const s of doc.sections) {
    if (TOC_EXCLUDED.has(s.name)) continue;
    lines.push(..._tocEntryLines(s.name, s.lines));
  }
  lines.push('');
  return lines;
}
