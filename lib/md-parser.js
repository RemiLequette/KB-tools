...
// subsection). The leading H1 must match doc.title exactly.
// ---------------------------------------------------------------------------
...
function _parsePath(doc, targetPath) {
  ...
  if (segments[0] !== doc.title) return null;
...
function getSections(doc) {
  const result = [];
  for (const s of doc.sections) {
    result.push({ path: `${doc.title}/${s.name}`, level: 2, content: s.lines.join('\n') });
    for (const sub of s.subsections) {
      result.push({ path: `${doc.title}/${s.name}/${sub.name}`, level: 3, content: sub.lines.join('\n') });
    }
  }
  return result;
}
