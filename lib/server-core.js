 ...
 * Returns { status, content } where content is a Buffer of the raw file
 * bytes — not decoded as text. Text consumers (JSON, Markdown) get back
 * the exact original bytes when re-serialized, since the file was written
 * as utf8 in the first place; binary consumers (images, etc.) are no
 * longer corrupted by a forced utf8 decode/re-encode round trip.
 *
 * @param {string} filePath - absolute path
 * @param {string[]} allowedRoots
 * @returns {{ status: number, content?: Buffer, error?: string }}
 */
...
export { safePath, readFile, writeFile, deleteFile, listDir, parseAllowedRoots, parsePort };
