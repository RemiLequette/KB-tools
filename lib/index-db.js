 ...
 * @param {{ name: string, content: string }[]} sections - Parsed sections.
 */
export function upsertDocument(db, repo, filePath, title, loadWhen, mtime, sections) {
  db.transaction(() => {
    ...
    const insertSection = db.prepare(
      'INSERT INTO sections (doc_id, name, content) VALUES (?,?,?)'
    );
    const insertFts = db.prepare(
      'INSERT INTO sections_fts (rowid, name, content, doc_id) VALUES (?,?,?,?)'
    );

    for (const { name, content } of sections) {
      const { lastInsertRowid: secId } = insertSection.run(docId, name, content);
      insertFts.run(secId, name, content, docId);
    }
  })();
}
